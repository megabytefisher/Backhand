...
namespace Backhand.Common.BinarySerialization.Internal
{
    internal static class ReadOnlySequenceExpressions<T>
    {
        ...
        private static readonly MethodInfo GetOffset = typeof(ReadOnlySequence<T>).GetMethod(nameof(ReadOnlySequence<T>.GetOffset), new[] { typeof(SequencePosition) }) ?? throw new Exception("Couldn't find method GetOffset");
        ...
        public static Expression GetGetOffsetExpression(Expression readOnlySequence, Expression position) => Expression.Call(readOnlySequence, GetOffset, position);
